...
<body>
  ...
  <div class="container">
    ...
    <article itemscope itemtype="https://schema.org/BlogPosting">
      ...
      <div class="related-network" style="background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);padding:24px;margin:40px 0">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px">
          ...
          <a href="https://brightlane.github.io/GlobalCampaignHub/" rel="noopener" style="display:block;background:var(--bg3);border:1px solid var(--border);border-radius:8px;padding:14px 16px;color:var(--accent);text-decoration:none">
            ...
            <div style="font-size:0.9rem;font-weight:600">Global Campaign Hub — 70 Merchants →</div>
          </a>
        </div>
      </div>
...
async function main() {
  ...
  for (const kw of keywords) {
    ...
    // Translate in parallel chunks of 4 — ~4x faster
    const otherLangs = langList.filter(l => l.code !== 'en');
    const CHUNK_SIZE = 4;
    console.log(`\n🌍  Translating to ${otherLangs.length} languages (parallel, 4 at a time)...`);
