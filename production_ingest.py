...
def full_production_run():
    ...
    cursor = conn.cursor()
    cursor.execute("PRAGMA synchronous = OFF") # Speed boost
    cursor.execute("PRAGMA journal_mode = MEMORY") # Speed boost
    
    feed_dir = 'feeds/'
    # Map your 17 approved MIDs to the keys in your JSON
    mid_map = {
        ...
        if filename.endswith(".csv"):
            ...
            with open(os.path.join(feed_dir, filename), 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if len(batch) >= 5000: # Insert in batches for stability
            ...
            conn.commit()
    conn.close()
    print("🚀 PRODUCTION INGEST COMPLETE.")
